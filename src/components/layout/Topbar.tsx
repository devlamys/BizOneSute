...
import { Search, Plus, Bell, HelpCircle, Sun, Moon, Menu, Building2, ChevronDown, User, Settings, Users, LogOut } from 'lucide-react';
...
export function Topbar({ onMenu, onToggleCollapse, onSearch, onQuick, onNotif }: any) {
  ...
  return (
    <header className="h-14 shrink-0 bg-white dark:bg-[#161717] border-b border-gray-200 dark:border-gray-800 flex items-center gap-2 px-3">
      ...
      <button onClick={onSearch} className="hidden md:flex items-center gap-2 h-9 px-4 w-[280px] rounded-full border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-[#161717] text-[13px] text-gray-500 hover:border-gray-400">
        <Search size={15} /> Search customers, invoices… <kbd className="ml-auto text-[11px] bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-full px-1.5 py-0.5">Ctrl K</kbd>
      </button>
      <button onClick={onSearch} className="md:hidden p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800" aria-label="Search"><Search size={18} /></button>
