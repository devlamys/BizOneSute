    ...
    <div>
      ...
      {tab === 'Pipeline' ? (
        <div className="grid md:grid-cols-5 gap-2">
          {stages.map(s => <div key={s} className="erp-card p-2"><div className="text-[11px] font-bold uppercase text-gray-500 px-1 py-1">{s} ({leads.filter(l => l.stage === s).length})</div><div className="space-y-2">{leads.filter(l => l.stage === s).map(l => <div key={l.name} className="p-2.5 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#2E2F2F]"><div className="text-[12.5px] font-semibold">{l.name}</div><div className="text-[11.5px] text-gray-500">{l.contact} · {l.source}</div><div className="text-[12px] font-bold mt-1">{fmtINR(l.value)}</div></div>)}</div></div>)}
        </div>
